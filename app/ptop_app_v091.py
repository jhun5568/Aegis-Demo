    ...
    def _create_category_purchase_order(self, category, items, supplier_name, 
        ...
        try:
            with st.spinner(f"{category} → {supplier_name} 발주서 생성 중..."):
                # Normalize items to ensure required keys exist
                norm_items = []
                try:
                    for it in (items or []):
                        ...
                        if it.get('is_header'):
                            continue
                        name = it.get('material_name') or it.get('품목') or it.get('item_name') or it.get('model_name') or ''
                        if not str(name).strip():
                            continue
                        standard = it.get('standard') or it.get('규격') or ''
                        unit = it.get('unit') or it.get('단위') or 'EA'
                        qty = it.get('quantity') if 'quantity' in it else it.get('수량')
                        try:
                            qty = float(qty) if qty not in (None, '') else 0.0
                        except Exception:
                            qty = 0.0
                        model_ref = it.get('model_reference') or it.get('모델') or it.get('모델참조') or ''
                        norm_items.append({
                ...
                excel_buffer = self._create_single_purchase_order_by_category(
                    quotation_data, norm_items, delivery_location, 
                ...
                if excel_buffer:
                    ...
                    st.download_button(
                        ...
                        args=(self.tenant_id, category, supplier_name, items, ((st.session_state.get('last_material_data') or {}).get('site_info') or {}).get('project_id'))
                    )
